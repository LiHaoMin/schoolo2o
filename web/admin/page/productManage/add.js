...
layui.config({
    ...
    $.get(BASE_URL + '/shop/list2',function(data){
        if(data.success){
            for(var i = 0;i < data.data.length; i++){
                $('#shop').append('<option value="'+data.data[i].id+'">'+data.data[i].shopName+'</option>');
            }
            form.render('select');
        }else{
    ...
    var images = [];
 	form.on("submit(add)",function(data){
        ...
        data.field.productImages = images;
        $.ajax({
            type : "POST",
    ...
    $('#imgs').change(function (obj) {
        var fileObj = imgs.files[0]; // js 获取文件对象
        if (typeof (fileObj) == "undefined" || fileObj.size <= 0) {
            layer.msg("请选择图片");
            return;
        }
