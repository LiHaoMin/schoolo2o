layui.config({
	...
	$.get(BASE_URL + '/area/list',function(data){
		if(data.success){
			...
			laypage({
				...
				pages: app.pageTotal,
				jump: function(obj){
					$.get(BASE_URL + '/area/list',{page:obj.curr},function(data){
						if(data.success){
							app.dataList = data.data;
							app.pageTotal = data.total;
	...
	$(".batchDel").click(function(){
		...
		if($checkbox.is(":checked")){
			layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
				...
				$.get(BASE_URL + '/area/delete',{ids:ids.substring(0,ids.length - 1)},function(data){
					if(data.success){
	...
	$("body").on("click",".del",function(){  //删除
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
			...
			$.get(BASE_URL + '/area/delete',{ids:_this.attr("data-id")},function(data){
					if(data.success){
						layer.msg(data.errorMsg);
						_this.parents("tr").remove(); 
					}else{
